...
df_main_tbl=spark.sql("""select * from stage_load.t_care_ca_rel_main""")
df_prev_tbl=spark.sql("""select * from stage_load.t_care_ca_rel_prev""")
df_src_tbl=spark.sql("""select * from stage_load.t_care_ca_rel_src""")
...
df_new_records=df_new_records.withColumn("eff_strt_dt", lit(eff_strt_dt)).withColumn("eff_end_dt", lit(eff_end_dt)).withColumn("active_ind", lit("A"))

# create temp table only with new records. 
df_new_records.createOrReplaceTempView("temp_new_records")

#spark.sql("""insert into table stage_load.t_care_ca_rel_main select * from temp_new_records """)
...
-- src.run_d <> prev.run_d
-- src.hdfs_insert_tmst <> prev.hdfs_insert_tmst
-- src.hdfs_update_tmst <> prev.hdfs_update_tmst
-- src.hdfs_update_user <> prev.hdfs_update_user
""")

#add eff_strt_dt and eff_end_dt wihich is high end date.
df_update_records=df_update_records.withColumn("eff_strt_dt", lit(eff_strt_dt)).withColumn("eff_end_dt", lit(eff_end_dt)).withColumn("active_ind", lit("A"))
...
df_update_exstng_records=df_update_exstng_records.withColumn("active_ind", lit("I")).withColumn("eff_end_dt", lit(curr_dt))
#rearrange the column sequence
df_update_exstng_records=df_update_exstng_records.select(["prtn_id","cus_id","fbsi_firm_c","fbsi_brch_c","fbsi_base_c","fid_acc_id","rel_ty_c","seq_n","prnc_nm_c","multi_co_n","cre_tmst","cre_user","upd_tmst","upd_usr","email_ad_of_rec_c","prm_cus_c","ibmsnap_intentseq","ibmsnap_commitseq","ibmsnap_operation","ibmsnap_logmarker","rel_to_tre_c","tst_cont_opt_out_c","run_d","hdfs_insert_tmst","hdfs_update_tmst","hdfs_update_user","eff_strt_dt","eff_end_dt","active_ind"])
...
df_del_records=df_del_records.withColumn("eff_end_dt", lit(curr_dt)).withColumn("active_ind", lit("I"))

#rearrange the column sequence
df_del_records=df_del_records.select(["prtn_id","cus_id","fbsi_firm_c","fbsi_brch_c","fbsi_base_c","fid_acc_id","rel_ty_c","seq_n","prnc_nm_c","multi_co_n","cre_tmst","cre_user","upd_tmst","upd_usr","email_ad_of_rec_c","prm_cus_c","ibmsnap_intentseq","ibmsnap_commitseq","ibmsnap_operation","ibmsnap_logmarker","rel_to_tre_c","tst_cont_opt_out_c","run_d","hdfs_insert_tmst","hdfs_update_tmst","hdfs_update_user","eff_strt_dt","eff_end_dt","active_ind"])
...
spark.sql("""INSERT OVERWRITE TABLE stage_load.t_care_ca_rel_main select * from final_table""")